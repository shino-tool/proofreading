...
def analyze_serp_snippets(items, keyword):
    """
...
def mock_serp_analysis(keyword):
    """
...
def process(text, target_keyword, custom_search_engine_id=None):
    """
    ...
    api_key = os.environ.get("GOOGLE_API_KEY") # We assume using same key or separate? 
    # Usually CSE needs a separate API Key configured for Search, but users might reuse one if enabled.
    # We will use the main one for now, but in UI we might ask for specific one if needed.
    ...
    if custom_search_engine_id and api_key and target_keyword:
        ...
        if items:
            related_keywords_str = analyze_serp_snippets(items, target_keyword)
        else:
            related_keywords_str = mock_serp_analysis(target_keyword)
    else:
    ...
    return get_gemini_response(prompt, temperature=0.5) # Lower temperature for SEO precision
